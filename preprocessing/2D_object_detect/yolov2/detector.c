...
void test_detector_folder(char *datacfg, char *cfgfile, char *weightfile, char *input_folder, char *output_folder, float thresh, float hier_thresh)
{
    ...
    int max_labels=net.layers[net.n-1].classes;
    int ii=0;int jj=0;
    for (ii=0; ii < max_labels; ii++)
    { 
      for (jj=0;jj<255;jj++)
      {
	if (names[ii][jj]=='\0')
	    break;
	else if (names[ii][jj]==' ')  // find space, replace it by '_'
	    names[ii][jj]='_';
      }
//       printf("class name %s \n",names[ii]);
    }
...
// 	  printf("save to txt:   %s \n",result_file);
	...
	{
	  draw_detections(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes);   // if want to show classes, prob in terminal. See inside function.
	  save_image(im, output_file);
	}
	else
	{
	  draw_save_detections(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes,fp, output_file);
	}
