...
public:
    ...
    // Used to track the local map (Tracking  local map)
    int SearchByProjection(Frame &F, const std::vector<MapPoint*> &vpMapPoints, const float th=3);

    // Project MapPoints tracked in last frame into the current frame and search matches.
    // Used to track from previous frame (Tracking  motion model)
    ...
    // Used in relocalisation (Tracking Relocalization)
    ...
    // Search matches between MapPoints in a KeyFrame and ORB in a Frame. No projection, only use ORB descriptors
    // Brute force constrained to ORB that belong to the same vocabulary node (at a certain level)
    // Used in Relocalisation and Loop Detection.
    int SearchByBoW(KeyFrame *pKF, Frame &F, std::vector<MapPoint*> &vpMapPointMatches); // (tracking)
    int SearchByBoW(KeyFrame *pKF1, KeyFrame* pKF2, std::vector<MapPoint*> &vpMatches12);

    // Matching for the Map Initialization (only used in the monocular case), no projection, directly match features in window area in image space.
    ...
    // Project MapPoints into KeyFrame and search for duplicated MapPoints. (localmapping)
