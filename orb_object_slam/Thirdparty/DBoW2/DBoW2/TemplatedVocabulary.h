...
public:
  ...
  /**
   * Saves the vocabulary from a text file
   * @param filename
   */    
...
bool TemplatedVocabulary<TDescriptor,F>::loadFromBinaryFile(const std::string &filename)
{
    ...
    int row_size = 1082072;
    int column_size = 35;
    int total_number = row_size*column_size;
    
    float *all_data = (float*)malloc(total_number * sizeof(float));
    f.read((char*)all_data, total_number*sizeof(float));
	...
	int pid = all_data[i*column_size+0];
        m_nodes[nid].parent = pid;
        m_nodes[pid].children.push_back(nid);	
	
	int nIsLeaf = all_data[i*column_size+1];

        stringstream ssd;
        for(int iD=0;iD<F::L;iD++)
        {
            ssd << all_data[i*column_size+2+iD] << " ";
	}
        F::fromString(m_nodes[nid].descriptor, ssd.str());

	m_nodes[nid].weight = all_data[i*column_size + column_size-1];
